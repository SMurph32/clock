	...
	int show_alarm=0, lock = 0, press = 0, button=0, count_add=1;

	uint32_t count=0, alarm=0;
	...
	void read_enc(knob){
		...
		uint32_t *target;//isolates 2 digits relevant to encoder
		...
		switch(enc_state){
			...
			case 1://one click right, three clicks left
				...
				if(temp == 3){
					...
					*target = (*target - count_add);
					if(*target<0) *target = MODCAP+*target;			
				}
				break;
			...
			case 3://three clicks right, one click left
				if(temp == 3){
					enc_state++;
					*target = (*target + count_add)%MODCAP;			
				}
				if(temp == 0) enc_state--;
				break;
...
//		count = ((count + 1)%MODCAP);//bound the count to 0 - 1023
...
void segsum(uint32_t sum, int c_mode) {
	int num_d=0, i;
	long hour, min;
	int temp, temp2, am;
	temp = sum;
...
uint8_t main()
{
	...
	while(1)
	{
		...
		if(show_alarm) 	segsum(alarm, TWENTYFOURHOUR);
		else 		segsum(count, TWENTYFOURHOUR);
